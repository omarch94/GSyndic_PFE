...
@section('title', 'Ajouter un facture')

@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau facture</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/factures" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="">

                    <div class="form-group mb-4">
                        <label class="mb-2" for="numero_facture">Numéro de facture : </label>
                        <input name="numero_facture" id="numero_facture" type="text" class="form-control @error('numero_facture') is-invalid @enderror" value="{{ old('numero_facture') }}">
                    ...
                    <div class="row mb-4">
                        <div class="form-group col-6">
                            ...
                            <input name="date_emission" id="date_emission" type="date" class="form-control @error('date_emission') is-invalid @enderror" value="{{ old('date_emission') }}">
                            @error('date_emission')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group col-6">
                            ...
                            <input name="date_echeance" id="date_echeance" type="date" class="form-control @error('date_echeance') is-invalid @enderror" value="{{ old('date_echeance') }}">
                            @error('date_echeance')
                            <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="montant_total" id="montant_total" type="text" class="form-control @error('montant_total') is-invalid @enderror" value="{{ old('montant_total') }}">
                        @error('montant_total')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
...
@section('js')
    <script>
        $(document).ready(function() {
            var selectedCoproprieteId = '{{ old('copropriete_id') }}';
            var selectedChargeId = '{{ old('charge_id') }}';
