...
@section('title', 'Ajouter un resolution')

@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau resolution</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/resolutions" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="">
                    <div class="form-group mb-4">
                        ...
                        <select name="reclamation_id" id="reclamation_id" class="form-control @error('reclamation_id') is-invalid @enderror">
                            ...
                            @foreach($reclamations as $reclamation)
                                <option value="{{$reclamation->id}}" @selected(old('reclamation_id') == $reclamation->id)>{{$reclamation->designation}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="date_resolution" id="date_resolution" type="date" class="form-control @error('date_resolution') is-invalid @enderror" value="{{ old('date_resolution') }}">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="utilisateur_id" id="utilisateur_id" class="form-control @error('utilisateur_id') is-invalid @enderror">
                            ...
                            @foreach($fournisseurs as $fournisseur)
                                <option value="{{$fournisseur->id}}" @selected(old('utilisateur_id') == $fournisseur->id)>{{$fournisseur->nom}} {{$fournisseur->prenom}}</option>
                            @endforeach
                        </select>
                        @error('ville_id')
                        <div class="invalid-feedback">{{ $message }}</div>
