...
@section('title', 'Ajouter un reclamation')

@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau reclamation</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/reclamations" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="">
                    <div class="form-group mb-4">
                        ...
                        <select name="type_reclamation_id" id="type_reclamation_id" class="form-control @error('type_reclamation_id') is-invalid @enderror">
                            ...
                            @foreach($type_reclamations as $type_reclamation)
                                <option value="{{$type_reclamation->id}}" @selected(old('type_reclamation_id') == $type_reclamation->id)>{{$type_reclamation->nom}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <input name="date_reclamation" id="date_reclamation" type="date" class="form-control @error('date_reclamation') is-invalid @enderror" value="{{ old('date_reclamation') }}">
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="canal_id" id="canal_id" class="form-control @error('canal_id') is-invalid @enderror">
                            ...
                            @foreach($canals as $canal)
                                <option value="{{$canal->id}}" @selected(old('canal_id') == $canal->id)>{{$canal->nom}}</option>
                            @endforeach
                        </select>
                        @error('ville_id')
                        <div class="invalid-feedback">{{ $message }}</div>
