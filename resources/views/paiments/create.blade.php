...
@section('content_header')
    <div class="header my-4 d-flex justify-content-between">
        <h4 class="">Ajouter un nouvau paiment</h4>
...
@section('content')
    ...
    <div class="section_form">
        <div class="p-5 bg-white">
            <form action="/paiments" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="">

                    <div class="form-group mb-4">
                        <label class="mb-2" for="date_paiment">Date de paiment : </label>
                        <input name="date_paiment" id="date_paiment" type="date" class="form-control @error('date_paiment') is-invalid @enderror" value="{{ old('date_paiment') }}">
                        @error('date_paiment')
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="mode_paiment_id" id="mode_paiment_id" class="form-control @error('mode_paiment_id') is-invalid @enderror">
                            ...
                            @foreach($mode_paiments as $mode_paiment)
                                <option value="{{$mode_paiment->id}}" @selected(old('mode_paiment_id') == $mode_paiment->id)>{{$mode_paiment->nom}}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <select name="facture_id" id="facture_id" class="form-control @error('facture_id') is-invalid @enderror">
                            ...
                            @foreach($factures as $facture)
                                <option value="{{$facture->id}}" @selected(old('facture_id') == $facture->id)>{{$facture->numero_facture}}</option>
                            @endforeach
                        </select>
                        @error('facture_id')
                        <div class="invalid-feedback">{{ $message }}</div>
